...
export default class Model {
    ...
    getDataSimulator(hour, lat, day, month, year) {
        // Determino N
        const firstDate = Date.UTC(year, 0, 1)
        const dateNow = Date.UTC(year, month, day)
        const N = Math.floor((dateNow - firstDate) / (24 * 60 * 60 * 1000)) + 1

        // Determino los dias del año
        const totalDaysYear = new Date(year, 2, 0).getDate() == 29 ? 366 : 365

        // Determino Delta
        const x = (2 * Math.PI / totalDaysYear) * (N - 1 + ((hour - 12) / 24))
        const delta = 0.006918 - 0.399912 * Math.cos(x) + 0.070257 * Math.sin(x)
            - 0.006758 * Math.cos(2 * x) + 0.000907 * Math.sin(2 * x)
            - 0.002697 * Math.cos(3 * x) + 0.001480 * Math.sin(3 * x)
        
        // Determino omega y lat en radianes
        const omega = 15 * (hour - 12) * Math.PI / 180
        const latRad = lat * Math.PI / 180

        // Determino la elevacion y el azimuth
        const elevation = Math.asin(Math.cos(latRad) * Math.cos(omega) 
                            * Math.cos(delta) + Math.sin(latRad) * Math.sin(delta))
        let azimuth = (Math.cos(latRad) * Math.sin(delta)
                        - Math.cos(omega) * Math.sin(latRad) * Math.cos(delta)) / Math.cos(elevation)
        
        azimuth = Math.acos(constrain(azimuth, -1, 1))

        // Normalizamos el azimuth de 0 a 2PI
        azimuth = Math.sin(omega) > 0 ? (2 * Math.PI) - azimuth : azimuth
